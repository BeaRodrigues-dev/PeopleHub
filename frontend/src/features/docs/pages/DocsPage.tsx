  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3.5 }, maxWidth: 1200, mx: "auto" }}>
      ...
      <Stack direction="row" spacing={1} sx={{ mb: 2.5 }} flexWrap="wrap" useFlexGap>
        <Chip label="Todas" onClick={() => setCategory(null)} sx={{ fontWeight: 700, bgcolor: !category ? "primary.main" : "#ECF5EF", color: !category ? "#fff" : "text.secondary" }} />
        {DOCUMENT_CATEGORIES.map((c) => (
          <Chip key={c} label={c} onClick={() => setCategory(c)} sx={{ fontWeight: 700, bgcolor: category === c ? "primary.main" : "#ECF5EF", color: category === c ? "#fff" : "text.secondary" }} />
        ))}
      </Stack>
          ...
          {filtered.map((doc) => (
            <Box key={doc.id} sx={{ bgcolor: "background.paper", border: "1px solid", borderColor: "divider", borderRadius: 4, p: 2.25 }}>
              <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
                <Box sx={{ width: 40, height: 40, borderRadius: 2.5, bgcolor: "#ECF5EF", display: "grid", placeItems: "center", color: "primary.main" }}>
              ...
              <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mt: 1.5 }}>
                <Chip label={doc.category} size="small" sx={{ bgcolor: "#ECF5EF", color: "primary.main", fontWeight: 700 }} />
                <Typography
                  component="a"
                  href={doc.fileUrl}
                  target="_blank"
