 ...
 * La etapa terminal se resalta como "Contratados".
 */
export function StageCountsEditor({ vacancyId, stages }: { vacancyId: string; stages: PipelineStage[] }) {
  ...
  return (
    <Stack spacing={1.25}>
      ...
      <Stack spacing={1}>
        {ordered.map((stage, i) => (
          <Box key={stage.id} sx={{ bgcolor: stage.isTerminal ? "#E1F3EA" : "background.paper", border: "1px solid", borderColor: "divider", borderRadius: 3, p: 1.5 }}>
            <Stack direction="row" alignItems="center" spacing={1.5}>
              <Stack direction="row" spacing={0.75} alignItems="center" sx={{ width: { xs: 130, sm: 190 }, flexShrink: 0 }}>
                ...
                {stage.isTerminal && <Chip label="Contratados" size="small" sx={{ height: 18, fontSize: 9.5, fontWeight: 700, bgcolor: "#2FA36B", color: "#fff" }} />}
              </Stack>
              <Box sx={{ flex: 1, height: 10, bgcolor: "#EFEDFB", borderRadius: 2, overflow: "hidden" }}>
                <Box sx={{ width: `${Math.min(100, (valueFor(stage) / maxCount) * 100)}%`, height: "100%", bgcolor: stage.isTerminal ? "#2FA36B" : BAR_COLORS[i % BAR_COLORS.length] }} />
              </Box>
              <TextField
                type="number"
                size="small"
