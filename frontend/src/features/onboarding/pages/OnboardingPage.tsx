            ...
            {entries.map((entry) => (
              <Box
                ...
                sx={{
                  ...
                  bgcolor: selected?.id === entry.id ? "#EFF7F1" : "background.paper",
                }}
              >
                <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
                  ...
                  <Stack direction="row" alignItems="center" spacing={0.25}>
                    <Chip
                      label={entry.status}
                      size="small"
                      sx={{ fontWeight: 700, bgcolor: entry.status === "En progreso" ? "secondary.light" : entry.status === "Completado" ? "#E7F3EC" : "#ECF5EF", color: entry.status === "En progreso" ? "primary.dark" : entry.status === "Completado" ? "#5F9678" : "text.secondary" }}
                    />
                ...
                <LinearProgress variant="determinate" value={entry.progress} sx={{ height: 6, borderRadius: 3, bgcolor: "#E7F1EB" }} />
    ...
    <Box sx={{ bgcolor: "background.paper", border: "1px solid", borderColor: "divider", borderRadius: 4, p: 3 }}>
      ...
      <Stack spacing={2}>
        {(["before", "day1", "week1"] as const).map((phase) => {
          ...
          return (
            <Box key={phase} sx={{ border: "1px solid", borderColor: "#E7F1EB", borderRadius: 3, p: 2 }}>
